...
public class Download {
	...
	public void dowloadFile(int id) {
		...
		try {
			...
			sendMail.sendMail("17130256@st.hcmuaf.edu.vn", "File Error", "Error sql");
			e.printStackTrace();
		}
	}
	...
	public void downloadFileFromNas(int id, String fileFomatDownload, String host, Integer portName, String userName,
		...
		if (success != true) {
			sendMail.sendMail("17130256@st.hcmuaf.edu.vn", "File Error", "error connect");
			return;
		}
		...
		// Tạo scp để sử dụng đối tượng SSH
		CkScp scp = new CkScp();
		...
		if (success != true) {
			sendMail.sendMail("17130256@st.hcmuaf.edu.vn", "File Error", "Error download");
			return;
		}
		// ngắt kết nối server
		ssh.Disconnect();
	...
	public void insertLog(String fileName, int id, String status) {
		String sql = "insert into datawarehouse_configuration.`control.data_file`(file_name,data_config_id,file_status,dt_file_queued) value('"
				+ fileName + "'," + id + ",'" + status + "','" + Timestamp.valueOf(LocalDateTime.now()) + "')";
		try {
