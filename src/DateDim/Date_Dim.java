...
public class Date_Dim {
	...
	public void insertDate_Dim() {
		String readDate = config.readFileFromFolder("F:\\anh\\datawarehouse\\date_dim\\date_dim_without_quater4.csv");
	...
	public void insertMonth_Dim() {
		...
		Connection connection = config.connectDataStaging();
		String sql = "insert into datawarehouse_staging.month_dim " + "values(?,?,?,?,?)";
		try {
			PreparedStatement pre = connection.prepareStatement(sql);
			String[] data = readDate.split("\n");
			for (String d : data) {
